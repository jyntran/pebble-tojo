...
static GBitmap *s_background_bitmap_white;
static GBitmap *s_background_bitmap_black;
...
static void prv_default_settings() {
  ...
  settings.CrestColour = GColorWhite;
...
static void prv_update_display() {
  ...
  if (gcolor_equal(settings.CrestColour, GColorWhite)) {
    bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap_white);
  } else if (gcolor_equal(settings.CrestColour, GColorBlack)) {
    bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap_black);    
  }
...
static void main_window_unload(Window *window) {
  gbitmap_destroy(s_background_bitmap_white);
  gbitmap_destroy(s_background_bitmap_black);
...
static void prv_inbox_received_handler(DictionaryIterator *iter, void *context) {
  ...
  Tuple *ct_colour_t = dict_find(iter, MESSAGE_KEY_CrestColour);
  if (ct_colour_t) {
    settings.CrestColour = GColorFromHEX(tm_colour_t->value->int32);
  }
...
static void init() {
  ...
  s_background_bitmap_white = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_TOJO_WHITE_100);
  s_background_bitmap_black = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_TOJO_BLACK_100);
